...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int)
{
    ...
    while (!done)
    {
        ...
        // #region agent log
        // H1: macro flicker — Present gap >= 48ms means a visible frame drop / ghost flash.
        {
            static auto s_lastPresent = std::chrono::steady_clock::time_point{};
            static auto s_lastGapLog = std::chrono::steady_clock::time_point{};
            const auto nowPresent = std::chrono::steady_clock::now();
            if (s_lastPresent.time_since_epoch().count() != 0) {
                const int gapMs = static_cast<int>(
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        nowPresent - s_lastPresent).count());
                if (gapMs >= 48
                    && (s_lastGapLog.time_since_epoch().count() == 0
                        || nowPresent - s_lastGapLog >= std::chrono::milliseconds(500))) {
                    s_lastGapLog = nowPresent;
                    int gateWaiters = 0;
                    const char* gateHolder = nullptr;
                    engine.GetScanGateSnapshot(gateWaiters, gateHolder);
                    std::ofstream f("F:/Test/ARCs/debug-c190fb.log", std::ios::app);
                    if (f) {
                        const auto ts = std::chrono::duration_cast<std::chrono::milliseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
                        f << "{\"sessionId\":\"c190fb\",\"runId\":\"flicker-debug\",\"hypothesisId\":\"H1\","
                          << "\"location\":\"Project.cpp:Present\",\"message\":\"paint_gap\","
                          << "\"data\":{\"gapMs\":" << gapMs
                          << ",\"gateWaiters\":" << gateWaiters
                          << ",\"gateHolder\":\"" << (gateHolder ? gateHolder : "") << "\"}"
                          << ",\"timestamp\":" << ts << "}\n";
                    }
                }
            }
            s_lastPresent = nowPresent;
        }
        // #endregion
