    ...
    std::vector<AimTarget>& targets)
{
    ...
    auto consider = [&](uintptr_t key, const PlayerCacheEntry& actor) {
        ...
        target.distanceM = actor.Distance;
        target.health = actor.health;
    ...
    std::vector<AimTarget>& targets)
{
    ...
    auto tryAddRobot = [&](uintptr_t key, const WorldCacheEntry& robot) {
        ...
        target.distToCenter = distToCenter;
        target.score = totalScore;
        target.distanceM = robot.Distance;
        target.health = robot.health;
        target.isRobot = true;
...
void Engine::AimAssistence()
{
    ...
    static uint64_t s_reactionLockKey = 0;
    static uint64_t s_reactionUntilMs = 0;
    static uint64_t s_overshootKey = 0;
    static float s_overshootPx = 0.f;
    static int s_overshootTicksLeft = 0;
    ...
    // Anti-detection: score noise
    if (var::enable_randomization && var::random_score_noise_pct > 0.f) {
        static std::mt19937 s_noiseRng{ std::random_device{}() };
        std::uniform_real_distribution<float> noiseDist(-var::random_score_noise_pct, var::random_score_noise_pct);
        for (auto& t : allTargets)
            t.score *= (1.f + noiseDist(s_noiseRng) / 100.f);
    }
    ...
    // Anti-detection: tick skip only while far from crosshair (avoid mid-track stutter).
    {
        const float preDist = hypotf(dx, dy);
        if (var::enable_randomization && var::random_tick_skip_pct > 0.f && preDist > 40.f) {
            static std::mt19937 s_rng{ std::random_device{}() };
            std::uniform_real_distribution<float> dist(0.f, 100.f);
            if (dist(s_rng) < var::random_tick_skip_pct)
                return;
        }
    }

    // Anti-detection: lock-on reaction delay
    if (var::enable_randomization
        && (var::random_reaction_min_ms > 0 || var::random_reaction_max_ms > 0)) {
        if (s_reactionLockKey != lockedTarget) {
            s_reactionLockKey = lockedTarget;
            if (lockedTarget != 0) {
                static std::mt19937 s_reactRng{ std::random_device{}() };
                const int lo = (std::max)(0, var::random_reaction_min_ms);
                const int hi = (std::max)(lo, var::random_reaction_max_ms);
                std::uniform_int_distribution<int> reactDist(lo, hi);
                s_reactionUntilMs = nowMs + static_cast<uint64_t>(reactDist(s_reactRng));
            } else {
                s_reactionUntilMs = 0;
            }
        }
        if (lockedTarget != 0 && nowMs < s_reactionUntilMs)
            return;
    }

    // Anti-detection: once-per-lock micro-overshoot that settles over a few ticks
    if (var::enable_randomization && var::random_overshoot_pct > 0.f) {
        if (s_overshootKey != lockedTarget) {
            s_overshootKey = lockedTarget;
            s_overshootTicksLeft = 0;
            s_overshootPx = 0.f;
            static std::mt19937 s_overRng{ std::random_device{}() };
            std::uniform_real_distribution<float> overChance(0.f, 100.f);
            if (overChance(s_overRng) < var::random_overshoot_pct) {
                std::uniform_real_distribution<float> overPx(3.f, 10.f);
                s_overshootPx = overPx(s_overRng);
                s_overshootTicksLeft = 8;
            }
        }
        if (s_overshootTicksLeft > 0 && s_overshootPx > 0.1f) {
            const float dist = std::sqrt(dx * dx + dy * dy);
            if (dist > 0.1f) {
                const float settle = static_cast<float>(s_overshootTicksLeft) / 8.f;
                const float off = s_overshootPx * settle;
                dx += (dx / dist) * off;
                dy += (dy / dist) * off;
            }
            --s_overshootTicksLeft;
        }
    }

    float pullScale = 1.f;

    // Anti-detection: context-aware adjustments (long range + low target HP)
    if (var::enable_randomization && var::random_context_aware) {
        const float distM = bestTarget->distanceM;
        if (distM > 250.f)
            pullScale *= 0.70f;
        else if (distM > 150.f)
            pullScale *= 0.85f;
        if (!bestTarget->isRobot) {
            ...
            if (const auto it = playerCache.find(static_cast<uintptr_t>(bestTarget->entityKey));
                it != playerCache.end() && it->second.health < it->second.maxhealth * 0.4f) {
                const float healthFrac = it->second.health / (std::max)(it->second.maxhealth, 1.f);
                pullScale *= 0.5f + healthFrac * 0.5f;
            }
        }
    }
