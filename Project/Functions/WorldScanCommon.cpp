...
size_t NearRangeUncachedImpl(
    const std::vector<uintptr_t>& admitIndex,
    const Engine::CameraCache& cam,
    float maxDistM,
    float marginM,
    const std::function<bool(uintptr_t)>& isCached,
    size_t maxAdds,
    const std::function<bool(uintptr_t)>& tryAdd)
{
    if (maxAdds == 0 || admitIndex.empty())
        return 0;

    const float capM = maxDistM > 0.f ? maxDistM : var::kMaxDistanceSliderM;
    const float limitM = capM + (marginM > 0.f ? marginM : 0.f);
    if (limitM <= 0.f)
        return 0;

    const float limitCm = limitM * 100.f;
    const float limitSq = limitCm * limitCm;

    std::vector<uintptr_t> uncached;
    uncached.reserve(admitIndex.size());
    for (uintptr_t key : admitIndex) {
        if (!key || isCached(key))
            continue;
        uncached.push_back(key);
    }
    if (uncached.empty())
        return 0;
    ...
    std::vector<CacheRootScatterRow> rows;
    rows.reserve(kChunk);

    for (size_t base = 0; base < uncached.size() && added < maxAdds; base += kChunk) {
        const size_t end = (std::min)(base + kChunk, uncached.size());
        rows.clear();
        for (size_t i = base; i < end; ++i) {
            CacheRootScatterRow row{};
            row.actorKey = uncached[i];
            rows.push_back(row);
        }
        if (!ScatterReadActorRootPositions(rows))
            continue;

        for (const CacheRootScatterRow& row : rows) {
            if (added >= maxAdds)
                break;
            if (!row.posValid)
                continue;
            const Vector3 delta{
                row.worldPos.x - cam.Location.x,
                row.worldPos.y - cam.Location.y,
                row.worldPos.z - cam.Location.z};
            const float distSq = delta.x * delta.x + delta.y * delta.y + delta.z * delta.z;
            if (distSq > limitSq)
                continue;
            if (tryAdd(row.actorKey))
                ++added;
        }
    }
    return added;
...
size_t AppendNearRangeUncachedProbes(
    ...
    size_t maxAdds)
{
    return NearRangeUncachedImpl(
        admitIndex, cam, maxDistM, marginM, isCached, maxAdds,
        [&](uintptr_t key) { return probeSet.insert(key).second; });
}

size_t CollectNearRangeUncachedKeys(
    std::vector<uintptr_t>& outKeys,
    const std::vector<uintptr_t>& admitIndex,
    const Engine::CameraCache& cam,
    float maxDistM,
    float marginM,
    ...
    size_t maxAdds)
{
    ...
    seen.reserve(outKeys.size() + maxAdds);
    for (uintptr_t k : outKeys)
        seen.insert(k);

    const size_t added = NearRangeUncachedImpl(
        admitIndex, cam, maxDistM, marginM, isCached, maxAdds,
        [&](uintptr_t key) {
...
constexpr int kFlickerFixIteration = 20;
