...
bool Engine::IsPlayerWeaponEspLabel(const std::string& label) const
{
    if (label.empty())
        return false;
...
std::string Engine::ResolvePlayerActiveWeaponLabel(const PlayerCacheEntry& actor) const
{
    std::string active = actor.weaponName;
    std::string stowed0 = actor.stowedWeapon0;
    std::string stowed1 = actor.stowedWeapon1;
    if (!IsPlayerWeaponEspLabel(active) && active != "Unarmed")
        active.clear();
    if (!IsPlayerWeaponEspLabel(stowed0))
        stowed0.clear();
    if (!IsPlayerWeaponEspLabel(stowed1))
        stowed1.clear();
    ...
    if (active == stowed1)
        stowed1.clear();
    if (active.empty() || active == "Unarmed")
        return {};
    return active;
}
