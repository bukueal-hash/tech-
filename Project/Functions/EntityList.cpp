...
void Engine::EntityList()
{
    ...
    dbgNearBypass = static_cast<int>(WorldScan::AppendNearRangeUncachedProbes(
        probeSet, admitIndex, cam,
        static_cast<float>(var::esp_distance), WorldScan::kNearRangeMarginM,
        [&](uintptr_t a) { return localCache.contains(a); }));
    ...
    for (auto it = localCache.begin(); it != localCache.end(); )
    {
        ...
        // hide_allies suppresses world ESP only; allies stay in cache for radar/menu.
        ...
        if (actor.isAlly) {
            // EmbarkPlayerState::SquadSlotIndex @ 0x4d0 — ally radar arrow color.
            const int32_t allySlot = Memory::read<int32_t>(playerState + 0x4d0);
            if (allySlot >= 0)
                actor.squadIdx = static_cast<uint8_t>(std::clamp(allySlot, 0, 2));
        }
        ...
        // GitHub evicts allies when hide_allies; we keep them in cache for radar
        // but force Drawing=false so world ESP (box/skeleton) never paints them.
        actor.Drawing =
            actor.health >= 1.0f && !(actor.isAlly && var::hide_allies);
        ++it;
