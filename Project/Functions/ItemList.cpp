...
void Engine::ItemList()
{
    ...
    std::vector<uintptr_t> itemAdmitIndex;
    itemAdmitIndex.reserve(ctx.currentActors.size());
    for (uint64_t actorU64 : ctx.currentActors) {
        ...
        if (IsGroundPickupGoneSticky(key))
            continue;
        itemAdmitIndex.push_back(key);
    }

    const float lootMaxM = var::loot_distance > 0.f ? var::loot_distance : var::kMaxDistanceSliderM;
    std::vector<uintptr_t> nearFirst;
    dbgNearBypass = static_cast<int>(WorldScan::CollectNearRangeUncachedKeys(
        nearFirst, itemAdmitIndex, ctx.camera, lootMaxM, WorldScan::kNearRangeMarginM,
        [&](uintptr_t k) { return localCache.contains(k); }));

    std::unordered_set<uintptr_t> nearSet(nearFirst.begin(), nearFirst.end());
    std::vector<uintptr_t> scanOrder;
    scanOrder.reserve(itemAdmitIndex.size());
    for (uintptr_t k : nearFirst)
        scanOrder.push_back(k);
    for (uintptr_t k : itemAdmitIndex) {
        if (!nearSet.contains(k))
            scanOrder.push_back(k);
    }

    for (uintptr_t key : scanOrder) {
        ++dbgScanned;
    ...
    int dbgPickupSnapshot = 0;
    for (const auto& [_, entry] : localCache) {
        if (!entry.Drawing)
            continue;
    ...
    if (var::show_debug_overlay) {
        std::cout << "[debugItem] scanned=" << dbgScanned
            ...
            << " nearBypass=" << dbgNearBypass
            << " distHystHold=" << LastWorldFinalizeDistHystHold()
            << " posReRead=" << LastWorldFinalizePosReRead()
            << std::endl;
    }
