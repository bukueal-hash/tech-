...
static constexpr uint8_t kBotDistMissClearDrawing = 3;
...
void Engine::RobotList()
{
    ...
    if (doAdmission) {
        ...
        dbgNearBypass = static_cast<int>(WorldScan::AppendNearRangeUncachedProbes(
            probeSet, admitIndex, cam, maxDistM, WorldScan::kNearRangeMarginM,
        ...
        // Prio "new" = not yet admitted (localCache keys only — failed verify stays new).
            ...
            if (!visualOk) {
                ...
                if (wantEvict) {
                    ...
                    if (distanceSq <= maxDistSq) {
                        WorldScan::MissCounterClear(s_botDistMisses, key);
                        actor.Drawing = true;
                        ++dbgDrawing;
                    } else {
                        ++dbgDistSkip;
                        if (WorldScan::MissCounterShouldEvict(
                                s_botDistMisses, key, false, kBotDistMissClearDrawing)) {
                            actor.Drawing = false;
                        } else {
                            actor.Drawing = true;
                            ++dbgDistHystHold;
    ...
    s_admitPrevActors.clear();
    for (const auto& [admitKey, _] : localCache)
        s_admitPrevActors.insert(admitKey);
