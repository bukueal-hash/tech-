...
void CollectSmcRoots(Engine& eng, const Vector3& localPos, float radiusCm, std::vector<SmcEntry>& out)
{
    ...
    // #region agent log
    {
        ...
        if (s_lastCollect.time_since_epoch().count() == 0
            || now - s_lastCollect >= std::chrono::seconds(2)) {
            s_lastCollect = now;
            std::ofstream f("F:/Test/ARCs/debug-c190fb.log", std::ios::app);
            if (f) {
                const auto ts = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
                f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"VC\","
                  << "\"location\":\"CollisionVis.cpp:CollectSmcRoots\",\"message\":\"vis_collect\","
                  << "\"data\":{\"scanned\":" << scanN
                  << ",\"smc\":" << (int)out.size()
                  << ",\"radiusM\":" << (radiusCm / 100.f)
                  << ",\"wall\":" << wallN
                  << ",\"door\":" << doorN
                  << ",\"tree\":" << treeN
                  << ",\"other\":" << otherN
                  << "},\"timestamp\":" << ts << "}\n";
                f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"VC\","
                  << "\"location\":\"CollisionVis.cpp:CollectSmcRoots\",\"message\":\"vis_classify\","
                  << "\"data\":{\"wall\":" << wallN
                  << ",\"door\":" << doorN
                  << ",\"tree\":" << treeN
                  << ",\"other\":" << otherN
...
void RebuildFromSmc(Engine& eng, const std::vector<SmcEntry>& smc, ProbeStatus& probeOut)
{
    ...
    // #region agent log
    {
        ...
        if (f) {
            const auto ts = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count();
            f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"VG\","
              << "\"location\":\"CollisionVis.cpp:RebuildFromSmc\",\"message\":\"vis_geom\","
              << "\"data\":{\"smc\":" << (int)smc.size()
              << ",\"probeHits\":" << probeHits
              << ",\"tris\":" << (int)all.size()
              << ",\"probe\":" << (int)probeOut
              << ",\"missMesh\":" << missMesh
              ...
              << "},\"timestamp\":" << ts << "}\n";
            f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"V\","
              << "\"location\":\"CollisionVis.cpp:RebuildFromSmc\",\"message\":\"vis_rebuild_detail\","
              << "\"data\":{\"smc\":" << (int)smc.size()
              << ",\"probeHits\":" << probeHits
              << ",\"tris\":" << (int)all.size()
              << ",\"probe\":" << (int)probeOut
...
float DecryptRenderFloat(uint32_t encrypted, uint32_t key)
{
...
void ProbeLastRenderTimes(Engine& eng)
{
    static auto s_last = std::chrono::steady_clock::time_point{};
    const auto now = std::chrono::steady_clock::now();
    if (s_last.time_since_epoch().count() != 0 && now - s_last < std::chrono::seconds(2))
        return;
    s_last = now;
    ...
    samples.reserve(4);
    {
        ...
        if (lock.owns_lock()) {
            for (const auto& [key, actor] : eng.playerCache) {
                if (!actor.Drawing || !actor.actorMesh)
                    continue;
                samples.push_back({ actor.actorMesh, actor.WorldPos, "player" });
                if (samples.size() >= 2)
                    break;
            }
        }
    }
    {
        ...
        if (lock.owns_lock()) {
            for (const auto& [key, actor] : eng.robotCache) {
                if (!actor.Drawing)
                    continue;
                const uintptr_t mesh = eng.GetActorSkeletalMesh(key);
                if (!mesh)
                    continue;
                samples.push_back({ mesh, actor.WorldPos, "bot" });
                if (samples.size() >= 4)
                    break;
            }
        }
    ...
    for (const Sample& s : samples) {
        if (!s.mesh || !Memory::IsValidPtrFast2(s.mesh))
            continue;
        const uint32_t rawLrt = Memory::read<uint32_t>(s.mesh + Offsets::Mesh_LastRenderTimeEnc);
        const uint32_t rawOn = Memory::read<uint32_t>(s.mesh + Offsets::Mesh_LastRenderTimeOnScreenEnc);
        const float lrt = DecryptRenderFloat(rawLrt, Offsets::Mesh_LastRenderTimeKey);
        const float lrtos = DecryptRenderFloat(rawOn, Offsets::Mesh_LastRenderTimeOnScreenKey);

        const uintptr_t worldPrivate = Memory::read<uintptr_t>(
            s.mesh + Offsets::UActorComponent_WorldPrivate);
        double timeSeconds = 0.0;
        if (worldPrivate && Memory::IsValidPtrFast2(worldPrivate))
            timeSeconds = Memory::read<double>(worldPrivate + Offsets::UWorld_TimeSeconds);
        else if (gWorld)
            timeSeconds = Memory::read<double>(gWorld + Offsets::UWorld_TimeSeconds);

        const float delta = static_cast<float>(timeSeconds) - lrtos;
        const int decryptVis = (std::isfinite(lrtos) && std::isfinite(delta) && delta < 0.2f) ? 1 : 0;

        const float plainSubmit = Memory::read<float>(s.mesh + Offsets::LastSubmitTime);
        const float plainRender = Memory::read<float>(s.mesh + Offsets::LastRenderTime);
        const float plainOn = Memory::read<float>(s.mesh + Offsets::LastRenderTimeOnScreen);
        const int plainVis = (plainRender == plainOn
            && std::fabs(plainSubmit - plainRender) <= 0.06f) ? 1 : 0;

        int rayClear = -1;
        if (IsPlausibleWorldPos(cam) && IsPlausibleWorldPos(s.world))
            rayClear = RaycastLos(cam, s.world).clear ? 1 : 0;

        f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"LRT\","
          << "\"location\":\"CollisionVis.cpp:ProbeLastRenderTimes\",\"message\":\"vis_lrt_probe\","
          << "\"data\":{\"kind\":\"" << s.kind
          << "\",\"mesh\":" << s.mesh
          << ",\"rawLrt\":" << rawLrt
          << ",\"rawOn\":" << rawOn
          ...
          << ",\"delta\":" << delta
          << ",\"decryptVis\":" << decryptVis
          << ",\"plainSubmit\":" << plainSubmit
...
void TickRebuild(Engine& eng)
{
    ...
    // #region agent log
    {
        ...
        if (f) {
            ...
            f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"A\","
              << "\"location\":\"CollisionVis.cpp:TickRebuild\",\"message\":\"vis_rebuild\","
              << "\"data\":{\"ms\":" << ms << ",\"smc\":" << (int)smc.size()
              << ",\"probe\":" << (int)probe
              << ",\"radiusM\":" << RebuildRadiusM()
              << ",\"dmaExec\":" << dmaExec << ",\"dmaLast\":" << dmaLast
              << "},\"timestamp\":" << ts << "}\n";
        }
    }
    // #endregion
...
RayHit RaycastLos(const Vector3& from, const Vector3& to)
{
    ...
    if (blocked) {
        ...
        // #region agent log
        {
            static auto s_lastHit = std::chrono::steady_clock::time_point{};
            const auto now = std::chrono::steady_clock::now();
            if (s_lastHit.time_since_epoch().count() == 0
                || now - s_lastHit >= std::chrono::milliseconds(500)) {
                s_lastHit = now;
                std::ofstream f("F:/Test/ARCs/debug-c190fb.log", std::ios::app);
                if (f) {
                    const auto ts = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch()).count();
                    const float distM = len / 100.f;
                    f << "{\"sessionId\":\"c190fb\",\"runId\":\"vis\",\"hypothesisId\":\"VR\","
                      << "\"location\":\"CollisionVis.cpp:RaycastLos\",\"message\":\"vis_ray_hit\","
                      << "\"data\":{\"hitT\":" << bestT
                      << ",\"distM\":" << distM
                      << ",\"blocker\":\"" << BlockerClassName(bestCls) << "\""
                      << "},\"timestamp\":" << ts << "}\n";
                }
            }
        }
