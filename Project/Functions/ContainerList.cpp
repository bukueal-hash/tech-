...
void Engine::ContainerList()
{
    ...
    const float contPreAdmitM = (std::max)(var::container_distance_sp, var::loot_distance);
    dbgNearBypass = static_cast<int>(WorldScan::AppendNearRangeUncachedProbes(
        probeSet, admitIndex, ctx.camera, contPreAdmitM, WorldScan::kNearRangeMarginM,
        [&](uintptr_t key) { return localCache.contains(key); }));
    ...
    for (const auto& [admitKey, _] : localCache)
        s_contAdmitPrevActors.insert(admitKey);
