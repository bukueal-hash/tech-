...
struct Snapshot {
    ...
    float random_tick_skip_pct{};
    float random_score_noise_pct{};
    int random_reaction_min_ms{};
    int random_reaction_max_ms{};
    float random_overshoot_pct{};
    bool random_context_aware{};
...
Snapshot CaptureSnapshot()
{
    ...
    s.enable_randomization = var::enable_randomization;
    s.random_tick_skip_pct = var::random_tick_skip_pct;
    s.random_score_noise_pct = var::random_score_noise_pct;
    s.random_reaction_min_ms = var::random_reaction_min_ms;
    s.random_reaction_max_ms = var::random_reaction_max_ms;
    s.random_overshoot_pct = var::random_overshoot_pct;
    s.random_context_aware = var::random_context_aware;
...
bool SnapshotsEqual(const Snapshot& a, const Snapshot& b)
{
    return a.enableesp == b.enableesp &&
        ...
        a.enable_randomization == b.enable_randomization &&
        a.random_tick_skip_pct == b.random_tick_skip_pct &&
        a.random_score_noise_pct == b.random_score_noise_pct &&
        a.random_reaction_min_ms == b.random_reaction_min_ms &&
        a.random_reaction_max_ms == b.random_reaction_max_ms &&
        a.random_overshoot_pct == b.random_overshoot_pct &&
        a.random_context_aware == b.random_context_aware &&
...
static bool ApplyAimbotConfigKey(const std::string& key, const std::string& val)
{
    ...
    if (key == "enable_randomization") {
        var::enable_randomization = ParseBool(val, var::enable_randomization);
        return true;
    }
    if (key == "random_tick_skip_pct") {
        var::random_tick_skip_pct = std::clamp(static_cast<float>(std::atof(val.c_str())), 0.f, 10.f);
        return true;
    }
    if (key == "random_score_noise_pct") {
        var::random_score_noise_pct = std::clamp(static_cast<float>(std::atof(val.c_str())), 0.f, 30.f);
        return true;
    }
    if (key == "random_reaction_min_ms") {
        var::random_reaction_min_ms = std::clamp(std::atoi(val.c_str()), 0, 500);
        return true;
    }
    if (key == "random_reaction_max_ms") {
        var::random_reaction_max_ms = std::clamp(std::atoi(val.c_str()), 0, 500);
        return true;
    }
    if (key == "random_overshoot_pct") {
        var::random_overshoot_pct = std::clamp(static_cast<float>(std::atof(val.c_str())), 0.f, 30.f);
        return true;
    }
    if (key == "random_context_aware") {
        var::random_context_aware = ParseBool(val, var::random_context_aware);
        return true;
    }
    /* Triggerbot */
...
void WriteIni()
{
    ...
    file << "enable_randomization=" << (var::enable_randomization ? 1 : 0) << '\n';
    file << "random_tick_skip_pct=" << var::random_tick_skip_pct << '\n';
    file << "random_score_noise_pct=" << var::random_score_noise_pct << '\n';
    file << "random_reaction_min_ms=" << var::random_reaction_min_ms << '\n';
    file << "random_reaction_max_ms=" << var::random_reaction_max_ms << '\n';
    file << "random_overshoot_pct=" << var::random_overshoot_pct << '\n';
    file << "random_context_aware=" << (var::random_context_aware ? 1 : 0) << '\n';
