    ...
    if (ImGui::BeginTabBar("##visuals_tabs", ImGuiTabBarFlags_None))
    {
        if (ImGui::BeginTabItem("ESP"))
        {
            ...
            if (ArcMenuLayout::Checkbox("Hide allies", &var::hide_allies))
                RequestArcSlowCache();
            ArcMenuHoverTooltip(
                "No world ESP on teammates (box, skeleton, silhouette, names). Radar still shows allies.");
...
void DrawArcAimbotTab()
{
    ...
    ImGui::Separator();
    ImGui::TextUnformatted("Anti-detection randomization");
    ArcMenuLayout::Checkbox("Enable randomization", &var::enable_randomization);
    ArcMenuHoverTooltip("Master toggle for reaction delay, score noise, overshoot, tick skip, and context-aware pull.");
    ImGui::BeginDisabled(!var::enable_randomization);
    {
        float reactMin = static_cast<float>(var::random_reaction_min_ms);
        if (ArcMenuLayout::SliderFloat(
                "Reaction min (ms)",
                "##random_reaction_min_ms",
                &reactMin,
                0.f,
                500.f,
                "%.0f"))
            var::random_reaction_min_ms = static_cast<int>(reactMin);
        float reactMax = static_cast<float>(var::random_reaction_max_ms);
        if (ArcMenuLayout::SliderFloat(
                "Reaction max (ms)",
                "##random_reaction_max_ms",
                &reactMax,
                0.f,
                500.f,
                "%.0f"))
            var::random_reaction_max_ms = static_cast<int>(reactMax);
        if (var::random_reaction_max_ms < var::random_reaction_min_ms)
            var::random_reaction_max_ms = var::random_reaction_min_ms;
    }
    ArcMenuHoverTooltip("Random delay before first aim pull on a new lock.");
    ArcMenuLayout::SliderFloat(
        "Score noise %",
        "##random_score_noise_pct",
        &var::random_score_noise_pct,
        0.f,
        30.f,
        "%.0f");
    ArcMenuHoverTooltip("Random noise on candidate scores so pick is not always the exact best.");
    ArcMenuLayout::SliderFloat(
        "Overshoot %",
        "##random_overshoot_pct",
        &var::random_overshoot_pct,
        0.f,
        30.f,
        "%.0f");
    ArcMenuHoverTooltip("Chance once per new lock to overshoot a few pixels then settle back.");
    ArcMenuLayout::SliderFloat(
        "Tick skip %",
        "##random_tick_skip_pct",
        &var::random_tick_skip_pct,
        0.f,
        10.f,
        "%.0f");
    ArcMenuHoverTooltip("Chance to skip aim output only while far from crosshair (not mid-track).");
    ArcMenuLayout::Checkbox("Context-aware", &var::random_context_aware);
    ArcMenuHoverTooltip("Slower pull at long range and when the target is low HP.");
    ImGui::EndDisabled();
...
void DrawArcHelpTab()
{
    ...
    WrappedBulletText(
        "Anti-detection randomization: reaction delay, score noise, overshoot, tick skip, context-aware pull.");
