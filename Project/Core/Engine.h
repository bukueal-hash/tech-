...
public:
    ...
    void FinalizeWorldCacheMap(
        ...
        int& outDrawing);
    int LastWorldFinalizeDistHystHold() const;
    int LastWorldFinalizePosReRead() const;
    ...
    /** World overlay only (box/skeleton/silhouette). Respects hide_allies. */
    bool ShouldDrawPlayerWorldEsp(const PlayerCacheEntry& entry) const;
    /** Minimap blips for non-allies; allows radar-range when not Drawing. */
    bool ShouldDrawPlayerRadarBlip(const PlayerCacheEntry& entry) const;
    ...
    bool IsPlayerWeaponEspLabel(const std::string& label) const;
    /** Active weapon label for ESP/menu (equipped, with stowed fallback). */
    std::string ResolvePlayerActiveWeaponLabel(const PlayerCacheEntry& actor) const;
    ...
    void SnapshotPlayerMenuRows(std::vector<PlayerMenuRow>& out) const {
        out.clear();
        std::shared_lock<std::shared_mutex> lock(m_playerCacheMutex);
        out.reserve(playerCache.size());
        const uintptr_t localPawn = AcknowledgedPawn;
        for (const auto& [key, entry] : playerCache) {
            (void)key;
            if (!entry.APawn || entry.APawn == localPawn)
                continue;
            PlayerMenuRow row{};
            row.pawn = entry.APawn;
            row.name = entry.ActorName.empty() ? "Raider" : entry.ActorName;
            row.weapon = ResolvePlayerActiveWeaponLabel(entry);
            row.squadIdx = entry.squadIdx;
            row.enemyTeamId = entry.enemyTeamId;
            row.isAlly = entry.isAlly;
            row.isDead = entry.bIsDead;
...
public:
    ...
    EspRenderFrame m_espFrameBuffers[2]{};
    std::atomic<int> m_espFramePublished{ 0 };
    /** Legacy single buffer + lock — kept for raid reset only. */
    EspRenderFrame m_lastEspFrame{};
    mutable std::shared_mutex m_espFrameMutex;
    std::atomic<uint64_t> m_espFrameSeq{ 0 };

public:
    const EspRenderFrame& PublishedEspFrame() const {
        return m_espFrameBuffers[m_espFramePublished.load(std::memory_order_acquire)];
    }
