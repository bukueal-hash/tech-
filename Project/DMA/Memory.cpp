...
static bool LevelOwnedByWorld(VMM_HANDLE h, DWORD pid, uint64_t level, uint64_t world)
{
    ...
    uint64_t actors = 0;
    int32_t count = 0;
    if (!ReadU64(h, pid, cluster + kHelpContainerActors, actors)
        || Engine::LooksLikeUtf16Garbage(static_cast<uintptr_t>(actors)))
        return false;
    if (!ReadI32(h, pid, cluster + kHelpContainerActorCount, count))
        return false;
    return actors != 0 && count > 0 && count <= 10000;
}

/** help/sdk.txt: UWorld → PL 0x120 / LevelCollections+0x20. */
...
bool PCIMemory::RefreshModuleInfo()
{
    if (!hVMM || !processId || attachedExe.empty())
        return false;

    PVMMDLL_MAP_MODULEENTRY pEntry = nullptr;
    if (!VMMDLL_Map_GetModuleFromNameU(hVMM, processId, attachedExe.c_str(), &pEntry, 0) || !pEntry)
        return false;
